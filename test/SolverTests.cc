...
TEST_P(PowerBasedSolverTest, SolverWithShiftOption) 
{
    ...
    EXPECT_NEAR(eigenval.real(), largest.real() + shift, 1e-2) 
        << ">>> PowerSolver failed to estimate the real part of the eigenvalue correctly.";
    EXPECT_NEAR(eigenval.imag(), largest.imag(), 1e-2) 
        << ">>> PowerSolver failed to estimate the complex part of the eigenvalue correctly.";
}
